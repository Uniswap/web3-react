...
type OperaProvider = Provider & { isOpera?: boolean; isConnected?: () => boolean; providers?: OperaProvider[] }
...
export class Opera extends Connector {
    ...
    await (this.eagerConnection = import('@metamask/detect-provider')
      ...
      .then((provider) => {
        if (provider) {
          ...
          // edge case if e.g. opera and coinbase wallet are both installed
          if (this.provider.providers?.length) {
            this.provider = this.provider.providers.find((p) => p.isOpera) ?? this.provider.providers[0]
          }
  ...
  public async activate(desiredChainIdOrChainParameters?: number | AddEthereumChainParameter): Promise<void> {
    ...
    await this.isomorphicInitialize()
    if (!this.provider) return this.actions.reportError(new NoOperaError())
