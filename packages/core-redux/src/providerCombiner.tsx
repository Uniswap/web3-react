...
import type { Connector, Web3ReactStateUpdate, Web3ReactReduxStore } from '@web3-react/types'
...
// TODO - Goal is to combine all connectors slices into one store, merging users external slices into the store as well. This will allow users to use the redux useSelector() hook like useSelector(store => store.metaMask.chainId)

export function createReducerManager(initialReducers: Reducer) {
  // Create an object which maps keys to reducers
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const reducers: Record<string, any> = { ...initialReducers }
  ...
  return {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    reduce: (state: any, action: PayloadAction<Web3ReactStateUpdate>) => {
...
export function Web3ReactProvider({
  ...
  // const reducerManager = createReducerManager(combineReducers)
  // const store = configureStore({ reducer: reducerManager.reduce })

  // const blah = useSyncExternalStore(
  //   store.subscribe,
  //   store.getState,
  // )

  // const startingContext: ReactReduxContextValue = {
  //   store,
  //   subscription: { onStateChange: (action) => store.dispatch(action) },
  //   getServerState:
  // }
  ...
  // const blah = useStore(store, CHAIN_ID, null)
  // // store.dispatch
  // // store.getState
  // // store.replaceReducer
  // // store.subscribe

  // reducerManager.add('asyncState', asyncReducer)
