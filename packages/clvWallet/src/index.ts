...
export class ClvWallet extends Connector {
  ...
  constructor({ actions, options, onError }: ClvWalletConstructorArgs) {
  ...
  private async isomorphicInitialize(): Promise<void> {
    ...
    const provider = windowObj.clover as any
    if (provider) {
      this.provider = provider as ClvWalletProvider

      // handle the case when e.g. clvWallet and coinbase wallet are both installed
      ...
      this.provider.on('disconnect', (error: ProviderRpcError): void => {
        // 1013 indicates that ClvWallet is attempting to reestablish the connection
        // https://github.com/ClvWallet/providers/releases/tag/v8.0.0
        if (error.code === 1013) {
          console.debug('ClvWallet logged connection error 1013: "Try again later"')
          return
        }
  ...
  public async connectEagerly(): Promise<void> {
    ...
    try {
      ...
      // we should be able to use `cancelActivation` here, but on mobile, clvWallet emits a 'connect'
