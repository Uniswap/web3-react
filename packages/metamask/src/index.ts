...
export class MetaMask extends Connector {
  ...
  private cancelActivation?: (force?: boolean) => void

  constructor({ actions, options, onError }: MetaMaskConstructorArgs) {
    super(actions, onError)
    this.options = options
    console.log('MetaMask constructor', this.options)
  }
  ...
  private async sdkProviderWatcher(): Promise<void> {
    ...
    provider.on('connect', (_connectParam): void => {
      const { chainId } = _connectParam as ProviderConnectInfo
      if (!chainId) {
        return
      }
    ...
    provider.on('disconnect', (_error): void => {
    ...
    provider.on('chainChanged', (_chainId): void => {
      const chainId = _chainId as string
      this.actions.update({ chainId: parseChainId(chainId) })
    })

    provider.on('accountsChanged', (_accounts): void => {
  ...
  public async connectEagerly(): Promise<void> {
    ...
    try {
      ...
      if (!provider) return this.cancelActivation?.()
  ...
  public async activate(desiredChainIdOrChainParameters?: number | AddEthereumChainParameter): Promise<void> {
    const provider = this.sdk?.getProvider()
    if (!provider?.isConnected?.()) this.cancelActivation = this.actions.startActivation()
