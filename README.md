...
- `setError: (error: Error, { preserveConnector?: boolean, connectorName?: string }) => void`: Sets `context.error`, optionally preserving the current connector if `preserveConnector` is `true` (default `true`), or setting a `connectorName` (note that if you're doing this, `preserveConnector` must be `false`).
